...
RenderDelegate::CommitResources(pxr::HdChangeTracker *tracker)
{
    ...
    // called MarkAllRprimsDirty() directly during sprim sync.  Hydra collects
    // the rprim dirty list before sprim sync runs, so those bits arrived too
    // late — geometry kept its stale LightSet and went black.  Light::Sync()
    // now calls setPendingCategoryUpdate() instead and we apply the dirty
    // marking here so it takes effect on the next Execute cycle.
    if (mPendingCategoryUpdate) {
        mPendingCategoryUpdate = false;
        tracker->MarkAllRprimsDirty(
